...
- 2026-02-15 | NotebookLM의 프로젝트 트랙은 저장 중심보다 `자료 탐색/분석 -> L1 반영` 중심으로 운영 | 분석 결과가 실제 실행 문서에 연결되어야 가치가 발생
- 2026-02-15 | 공통 규칙 원천은 `team-standards`(Git)로 단일화하고, NotebookLM은 프로젝트별 자료 탐색/분석 전용으로 운영 | 규칙 관리 단일 원천을 유지해 동기화/충돌 비용을 줄인다
