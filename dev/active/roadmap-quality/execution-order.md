...
> Last Updated: 2026-02-26 (세션 5 — Wave 1~3 전체 완료, 품질 달성)
...
| **law_api_client 파싱 버그 수정** | **2026-02-26** | `_to_str()` 헬퍼로 str/list/dict/None 안전 변환 |
| **source_url enrichment** | **2026-02-26** | `_personalized_to_steps_payload()`에서 ActionKit 파일 URL 자동 할당 |
| **CATEGORY_TO_PHASE 확장** | **2026-02-26** | chapters 7-12 → "영업 인허가" 매핑 추가 |
| **seed_rag_vectors dedup 수정** | **2026-02-26** | name+category_id 기반 중복 체크 (같은 법령 다른 업종 허용) |
...
| 이슈 | 심각도 | 비고 |
|---|---|---|
| ~~ActionKitItem 누락으로 매칭 0건~~ | **해결됨** | `_ensure_actionkit_items()` 추가 |
| ~~ActionKitFile 미생성으로 다운로드 불가~~ | **해결됨** | ActionKitFile 자동 생성 |
| ~~law_api_client.py 파싱 버그~~ | **해결됨** | `_to_str()` 헬퍼 추가 |
