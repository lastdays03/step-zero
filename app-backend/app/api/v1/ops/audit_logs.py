...
from app.features.ops.application.audit_logs.service import (
    AuditLogList,
    list_audit_logs,
    list_ops_audit_logs,
)
...
async def get_audit_logs(
    ...
    date_from: Optional[datetime] = Query(
        None, alias="from", description="시작 날짜 (ISO)"
    ),
    date_to: Optional[datetime] = Query(
        None, alias="to", description="종료 날짜 (ISO)"
    ),
    page: int = Query(1, ge=1, description="페이지 번호"),
    size: int = Query(20, ge=1, le=500, description="페이지 사이즈"),
    limit: Optional[int] = Query(
        None, ge=1, le=500, description="최대 조회 건수 (page/size 대신 사용)"
    ),
    offset: Optional[int] = Query(
        None, ge=0, description="오프셋 (page/size 대신 사용)"
    ),
    ...
    user_ids = {item.admin_id for item in admin_result.items} | {
        row.user_id for row in ops_rows
    }
