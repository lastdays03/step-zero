...
class ProcessedLawData(BaseModel):
    ...
    guide_text: str = Field(
        ..., description="Founder-friendly practical guide (Markdown)"
    )
    law_reference: str = Field(
        ..., description="Format: [Law Name] Article X (Content)"
    )
...
class LawETLProcessor:
    ...
    async def process(self, law_data: LawData) -> ProcessedLawData:
        ...
        try:
            ...
            logger.error(
                f"Error processing law data: title={law_data.title}, error={e}"
            )
