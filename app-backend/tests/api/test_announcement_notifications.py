...
async def test_announcement_notification_trigger(client: AsyncClient):
    ...
    async with db.async_session() as session:
        ...
        # 3. 알림 생성 확인
        notif_query = select(Notification).where(Notification.link == f"/announcements/{announcement.id}")
...
async def test_duplicate_publication_no_duplicate_notifications(client: AsyncClient):
    ...
    async with db.async_session() as session:
        ...
        # 알림 개수 확인
        result = await session.execute(select(Notification).where(Notification.link == f"/announcements/{announcement.id}"))
